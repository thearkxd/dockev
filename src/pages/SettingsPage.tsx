							...
							<div className="bg-surface-dark/40 rounded-2xl border border-border-dark p-1">
								<div className="p-5 border-b border-border-dark/50 last:border-none flex flex-col sm:flex-row sm:items-center justify-between gap-4">
									...
									<div className="relative min-w-[200px]">
										...
										>
											{allIDEs.length === 0 ? (
												<option value="">No IDEs available</option>
											) : (
												allIDEs.map((ide) => (
						...
						<section className="flex flex-col gap-5">
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									<button
										onClick={async () => {
											if (window.dockevDetect?.installedIDEs) {
												try {
													const detectedIDEs =
														await window.dockevDetect.installedIDEs();
													if (detectedIDEs.length > 0) {
														// Get all existing IDE IDs
														const existingIds = new Set(
															allIDEs.map((ide) => ide.id)
														);
														...
														if (newIDEs.length > 0) {
															...
															toast.success("All IDEs are already added.");
														}
													} else {
														toast.error("No IDEs detected on your system.");
													}
												} catch (error) {
													console.error("Error detecting IDEs:", error);
													toast.error("Failed to detect IDEs.");
												}
											}
										}}
							...
							<div className="bg-surface-dark/40 rounded-2xl border border-border-dark overflow-hidden flex flex-col">
								{allIDEs.length === 0 ? (
									<div className="p-8 text-center text-text-secondary text-sm">
										No IDEs added yet. Click "Detect IDEs" or "New IDE" to add
										one.
									</div>
								) : (
									allIDEs.map((ide) => (
