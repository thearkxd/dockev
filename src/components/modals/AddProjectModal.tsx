...
export function AddProjectModal({
	...
	const [defaultIde, setDefaultIde] = useState(
		settingsStorage.getSettings().defaultIde || ""
	);
	...
	// Reset form when modal closes and update default IDE when modal opens
	useEffect(() => {
		if (isOpen) {
			setIsAnimating(true);
			// Set a random color when modal opens (user can change it)
			setColor(getRandomColor());
			// Update default IDE from settings when modal opens
			const settings = settingsStorage.getSettings();
			setDefaultIde(settings.defaultIde || "");
		} else {
			setName("");
			setPath("");
			setCategory("");
