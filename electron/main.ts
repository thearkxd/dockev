...
async function detectInstalledIDEs(): Promise<
	Array<{ id: string; name: string; command: string }>
> {
	const detectedIDEs: Array<{ id: string; name: string; command: string }> = [];
	const isWindows = process.platform === "win32";

	// Common IDE commands to check
	const ideCommands: Array<{ id: string; name: string; command: string }> = [
		...
		{ id: "android-studio", name: "Android Studio", command: "studio" },
	...
	for (const ide of ideCommands) {
		try {
			if (isWindows) {
				// On Windows, use 'where' command (works in cmd and PowerShell)
				execSync(`where ${ide.command}`, {
					stdio: "ignore",
					timeout: 2000
				} as { stdio: "ignore"; timeout: number });
			} else {
				// On macOS/Linux, use 'which' command
				execSync(`which ${ide.command}`, {
					stdio: "ignore",
					timeout: 2000
				});
			}
			// Command exists, add to detected IDEs
...
ipcMain.handle("detect:installedIDEs", async () => {
	try {
